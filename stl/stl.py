...
if _NUMBA_AVAILABLE:
    ...
    @njit(cache=True)
    def _rasterize_triangles_zmin_numba(occ, zmin, v1, v2, v3, min_x, min_y, cell_mm):
        rows, cols = occ.shape
        for i in range(v1.shape[0]):
            x1, y1, z1 = v1[i, 0], v1[i, 1], v1[i, 2]
            x2, y2, z2 = v2[i, 0], v2[i, 1], v2[i, 2]
...
def _rasterize_triangles_zmin_numpy(occ, zmin, v1, v2, v3, min_x, min_y, cell_mm):
    rows, cols = occ.shape
    for i in range(v1.shape[0]):
        x1, y1, z1 = v1[i]
        x2, y2, z2 = v2[i]
        ...
        z = w1 * z1 + w2 * z2 + w3 * z3
        zmin_block = zmin[np.ix_(ys, xs)]
        zmin_block = np.where(mask, np.minimum(zmin_block, z), zmin_block)
        zmin[np.ix_(ys, xs)] = zmin_block
        occ[np.ix_(ys, xs)] |= mask
...
def calculate_removed_volume_top_and_bottom(
    filename,
    *,
    cell_mm=2.0,
    chunk_size=200000,
    max_cells=10_000_000,
    progress_callback=None,
    bottom_percentile=99.9,
):
    """
    Оценка снятого материала сверху + глубины снизу по растровой сетке.
    Возвращает dict: removed_volume_mm3_top, relief_depth_mm_top, relief_depth_mm_bottom,
    z_stock, z_bottom, cell_mm, total_triangles, projected_triangles.
    """
    import tempfile
    ...
    for chunk_start, chunk_end, total_triangles, v1, v2, v3 in _iter_stl_chunks_auto(
        ...
        max_y = max(max_y, v1_xy[:, 1].max(), v2_xy[:, 1].max(), v3_xy[:, 1].max())
        z_stock = max(z_stock, v1_local[:, 2].max(), v2_local[:, 2].max(), v3_local[:, 2].max())
        z_bottom = min(z_bottom, v1_local[:, 2].min(), v2_local[:, 2].min(), v3_local[:, 2].min())
    ...
    if (
        not np.isfinite(min_x)
        or not np.isfinite(min_y)
        or not np.isfinite(z_stock)
        or not np.isfinite(z_bottom)
    ):
        return None
    ...
    if width <= 0 or height <= 0:
        return {
            'removed_volume_mm3_top': 0.0,
            'relief_depth_mm_top': 0.0,
            'relief_depth_mm_bottom': 0.0,
            'cell_mm': cell_mm,
            'total_triangles': total_triangles,
            'projected_triangles': 0,
            'z_stock': float(z_stock),
    ...
    if total_cells > max_cells:
        ...
        temp_zmax = tempfile.NamedTemporaryFile(suffix=".dat", delete=False)
        temp_zmax.close()
        temp_zmin = tempfile.NamedTemporaryFile(suffix=".dat", delete=False)
        temp_zmin.close()
        footprint = np.memmap(temp_footprint.name, dtype=np.bool_, mode="w+", shape=(rows, cols))
        zmax = np.memmap(temp_zmax.name, dtype=np.float32, mode="w+", shape=(rows, cols))
        zmin = np.memmap(temp_zmin.name, dtype=np.float32, mode="w+", shape=(rows, cols))
    else:
        footprint = np.zeros((rows, cols), dtype=np.bool_)
        zmax = np.zeros((rows, cols), dtype=np.float32)
        zmin = np.zeros((rows, cols), dtype=np.float32)
    ...
    for chunk_start, chunk_end, total_triangles, v1, v2, v3 in _iter_stl_chunks_auto(
        ...
        normals_local = np.cross(v2_local - v1_local, v3_local - v1_local)
        top_mask = normals_local[:, 2] > eps_normal
        bottom_mask = normals_local[:, 2] < -eps_normal

        if np.any(top_mask):
            v1_top = v1_local[top_mask]
            v2_top = v2_local[top_mask]
            v3_top = v3_local[top_mask]
            v1_top_xy = v1_top[:, :2]
            v2_top_xy = v2_top[:, :2]
            v3_top_xy = v3_top[:, :2]
            area2_top = np.abs(
                (v2_top_xy[:, 0] - v1_top_xy[:, 0]) * (v3_top_xy[:, 1] - v1_top_xy[:, 1])
                - (v2_top_xy[:, 1] - v1_top_xy[:, 1]) * (v3_top_xy[:, 0] - v1_top_xy[:, 0])
            )
            valid_top = area2_top > 1e-5
            if np.any(valid_top):
                v1_top_valid = v1_top[valid_top]
                v2_top_valid = v2_top[valid_top]
                v3_top_valid = v3_top[valid_top]
                if _NUMBA_AVAILABLE:
                    _rasterize_triangles_zmax_numba(footprint, zmax, v1_top_valid, v2_top_valid, v3_top_valid, min_x, min_y, cell_mm)
                else:
                    _rasterize_triangles_zmax_numpy(footprint, zmax, v1_top_valid, v2_top_valid, v3_top_valid, min_x, min_y, cell_mm)

        if np.any(bottom_mask):
            v1_bot = v1_local[bottom_mask]
            v2_bot = v2_local[bottom_mask]
            v3_bot = v3_local[bottom_mask]
            v1_bot_xy = v1_bot[:, :2]
            v2_bot_xy = v2_bot[:, :2]
            v3_bot_xy = v3_bot[:, :2]
            area2_bot = np.abs(
                (v2_bot_xy[:, 0] - v1_bot_xy[:, 0]) * (v3_bot_xy[:, 1] - v1_bot_xy[:, 1])
                - (v2_bot_xy[:, 1] - v1_bot_xy[:, 1]) * (v3_bot_xy[:, 0] - v1_bot_xy[:, 0])
            )
            valid_bot = area2_bot > 1e-5
            if np.any(valid_bot):
                v1_bot_valid = v1_bot[valid_bot]
                v2_bot_valid = v2_bot[valid_bot]
                v3_bot_valid = v3_bot[valid_bot]
                if _NUMBA_AVAILABLE:
                    _rasterize_triangles_zmin_numba(footprint, zmin, v1_bot_valid, v2_bot_valid, v3_bot_valid, min_x, min_y, cell_mm)
                else:
                    _rasterize_triangles_zmin_numpy(footprint, zmin, v1_bot_valid, v2_bot_valid, v3_bot_valid, min_x, min_y, cell_mm)

        mask_any = top_mask | bottom_mask
        if np.any(mask_any):
            v1_xy = v1_local[mask_any][:, :2]
            v2_xy = v2_local[mask_any][:, :2]
            v3_xy = v3_local[mask_any][:, :2]
            area2 = np.abs(
                (v2_xy[:, 0] - v1_xy[:, 0]) * (v3_xy[:, 1] - v1_xy[:, 1])
                - (v2_xy[:, 1] - v1_xy[:, 1]) * (v3_xy[:, 0] - v1_xy[:, 0])
            )
    ...
    valid_top = footprint & np.isfinite(zmax)
    if not np.any(valid_top):
        removed_volume_mm3_top = 0.0
        relief_depth_mm_top = 0.0
    else:
        diff_top = (z_stock - zmax)[valid_top]
        removed_volume_mm3_top = float(np.sum(diff_top) * (cell_mm ** 2))
        diff_top = diff_top[np.isfinite(diff_top)]
        if diff_top.size == 0:
            relief_depth_mm_top = 0.0
        else:
            relief_depth_mm_top = float(np.percentile(diff_top, depth_percentile))

    valid_bottom = footprint & np.isfinite(zmin)
    if not np.any(valid_bottom):
        relief_depth_mm_bottom = 0.0
    else:
        diff_bottom = zmin[valid_bottom] - z_bottom
        diff_bottom = diff_bottom[np.isfinite(diff_bottom)]
        if diff_bottom.size == 0:
            relief_depth_mm_bottom = 0.0
        else:
            relief_depth_mm_bottom = float(np.percentile(diff_bottom, bottom_percentile))

    return {
        'removed_volume_mm3_top': removed_volume_mm3_top,
        'relief_depth_mm_top': relief_depth_mm_top,
        'relief_depth_mm_bottom': relief_depth_mm_bottom,
        'cell_mm': cell_mm,
        'total_triangles': total_triangles,
        'projected_triangles': projected_triangles,
        'z_stock': float(z_stock),
